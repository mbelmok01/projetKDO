...
class ContribuerForm extends Form
{
	...
	protected $adapter;
	public function __construct(AdapterInterface $dbAdapter, $idMembre)
	{
		$membreId = $idMembre;
	
	
		$this->adapter =$dbAdapter;
		parent::__construct("Invitations Form");
		...
		$this->add(array(
				...
				'options' => array(
						'label' => 'Nom ',
						'value_options' => $this->getEvenements($idMembre),
				),
				'attributes' => array(
						'class'=> 'form-control',
						//'required' => 'required'
	...
	public function getEvenements($idMembre)
	{
			try {
			...
			foreach ($result as $res) {
				$selectData[$res['id']] = $res['nom'] . ' ' . $res['date_evenement'];
			}
			return $selectData;
		} catch (Exception $e) {
